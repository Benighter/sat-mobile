...
const EditPrayerTimesModal: React.FC<EditPrayerTimesModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
      <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl sm:max-w-2xl w-full max-h-[92dvh] sm:max-h-[90dvh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-3 sm:px-6 sm:py-4 rounded-t-2xl sm:rounded-t-xl flex items-center justify-between">
          <div>
        ...
        <div className="p-4 space-y-4 sm:p-6 sm:space-y-6">
          ...
          <div className="space-y-4">
            ...
            {days.map(({ key, label }) => {
              ...
              return (
                <div key={key} className={`p-3 sm:p-4 rounded-lg border-2 transition-all ${isDayDisabled ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div className="font-medium text-gray-700">{label}</div>
                    <button
                      type="button"
                      onClick={() => handleToggleDay(key)}
                      style={{ width: 56, height: 26, borderRadius: 13, flexShrink: 0, position: 'relative', border: 'none', cursor: 'pointer', transition: 'background 0.2s', background: isDayDisabled ? '#ef4444' : '#22c55e', padding: 0, overflow: 'hidden' }}
                    >
                      {/* Label text */}
                      <span style={{
                        position: 'absolute',
                        ...
                        color: 'white',
                        letterSpacing: 0.5,
                        left: isDayDisabled ? 'auto' : 8,
                        right: isDayDisabled ? 8 : 'auto',
                        userSelect: 'none',
                      }}>
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-gray-50 px-4 py-3 sm:px-6 sm:py-4 rounded-b-2xl sm:rounded-b-xl border-t border-gray-200">
          {/* Success message */}
          {saveSuccess && (

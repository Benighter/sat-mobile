...
| `setUserActiveStatus` | callable | Updates profile, Firebase Auth disabled state, and tokens | Existing tenant-scoped database RPC plus a future privileged Edge Function for Firebase Auth administration | P0 admin | Hybrid implemented; Firebase retained for identity side effect |
...
| `sendBirthdayEmail` | callable | Admin-triggered birthday email via secret providers | Authenticated Edge Function with admin authorization and project secrets | P1 email | Planned; no secrets copied |
| `sendBirthdayEmailHttp` | HTTP | Duplicate HTTP birthday email endpoint | Same Edge Function; remove duplicate route after verification | P2 duplicate | Planned |

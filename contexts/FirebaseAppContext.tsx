...
export const FirebaseAppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
      ...
      if (isDemotedFromGreenBacenta) {
        ...
        if (linkedFellowshipLeaders.length > 0) {
          ...
          (async () => {
            ...
            const { db: dbRef } = await import('../firebase.config');
            const { firebaseUtils } = await import('../services/firebaseService');
            const cid = firebaseUtils.getCurrentChurchId();
            if (cid) {
              try {
                const batchUnlink = wbFn(dbRef);
                linkedFellowshipLeaders.forEach(fl => {
                  batchUnlink.update(docFn(dbRef, `churches/${cid}/members`, fl.id), {
                    bacentaLeaderId: '',
                    lastUpdated: new Date().toISOString()
                  });
                });
                await batchUnlink.commit();
                console.log(`✅ De-linked ${linkedFellowshipLeaders.length} Red Bacenta(s) after demotion`);
              } catch (unlinkErr) {
                console.warn('⚠️ Failed to de-link Red Bacentas after Green Bacenta demotion:', unlinkErr);
              }
            }
          })();
  ...
  const deleteMemberHandler = useCallback(async (memberId: string) => {
    ...
    try {
      ...
      // If deleting a Green Bacenta (Bacenta Leader), auto de-link all their Red Bacentas
      if (memberToDelete.role === 'Bacenta Leader') {
        const linkedFellowshipLeaders = members.filter(
          m => m.role === 'Fellowship Leader' && m.bacentaLeaderId === memberId
        );
        if (linkedFellowshipLeaders.length > 0) {
          console.log(`🔗 De-linking ${linkedFellowshipLeaders.length} Red Bacenta(s) from deleted Green Bacenta ${memberId}`);
          // Fire-and-forget: update Firestore in background, optimistic update below
          (async () => {
            const { writeBatch: wbFn, doc: docFn } = await import('firebase/firestore');
            const { db: dbRef } = await import('../firebase.config');
            const { firebaseUtils } = await import('../services/firebaseService');
            ...
            if (cid) {
              try {
                ...
                await batchUnlink.commit();
                console.log(`✅ De-linked ${linkedFellowshipLeaders.length} Red Bacenta(s)`);
              } catch (unlinkErr) {
                console.warn('⚠️ Failed to de-link Red Bacentas from deleted Green Bacenta:', unlinkErr);
              }
            }
          })();
          // Optimistic UI: clear bacentaLeaderId for linked fellowship leaders
          setMembers(prev => prev.map(m =>
            m.role === 'Fellowship Leader' && m.bacentaLeaderId === memberId
              ? { ...m, bacentaLeaderId: '' }
              : m
          ));
        }

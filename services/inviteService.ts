...
export const inviteService = {
  ...
  // Get all invites created by an admin (with optional churchId fallback for promoted admins)
  getAdminInvites: async (adminUid: string, churchId?: string): Promise<AdminInvite[]> => {
    try {
      // Primary query: invites where this admin is the creator
      ...
      primarySnap.docs.forEach(d => {
        seen.add(d.id);
        invites.push({ id: d.id, ...d.data() } as AdminInvite);
      });

      // Fallback query: if churchId provided, also fetch accepted invites for the same church
      // This covers promoted admins whose adminInvites.createdBy may not yet be updated.
      if (churchId) {
